var Yl = {
    ...
    ,create : function(){ //사다리 생성
     ...
     for(var i=0; i<this.nNum; i++){
      ...
      this.arrGoDiv[i].innerHTML = '<button type="button" value="GO" onClick="Yl.start('+i+')">Go</button>';
      ...
      if( nEy<this.nHeight+100 ){
       ...
       for(var i=0; i<this.arrVerDiv.length; i++){
        if(nEx==parseInt(this.arrVerDiv[i].style.left)){      
         ...
         this.arrGoDiv[pno].innerHTML = '<button type="button" value="확인" onClick="Yl.start('+pno+')">확인<button>';
